...
dl <- d %>%
  select(psuid,age,agecat,pfmsp19,nie,t24) %>%
  gather(key=antigen,value=mfi,-psuid,-age,-agecat) %>%
  mutate(antigen = factor(antigen,levels=c("pfmsp19","nie","t24"))) %>%
...
# unstratified
pdist2 <- ggplot(data=dl,aes(x=logmfi,fill=antigen)) +
  facet_grid(~antigen) +
  geom_density(alpha=0.5,color=NA) +
  geom_vline(aes(xintercept = roccut),lty=1) +
  # geom_vline(aes(xintercept = mixcut),lty=2) +
...
cv_msp1 <- SuperLearner(Y=sld$logmfi,X=select(sld,age), SL.library = paste("SL.mgcv.k",4:20,sep=""))
cv_msp1
...
# Pf Malaria MSP-1
fit_msp1 <- mgcv::gam(logmfi~s(age,bs="cr",k=4) + s(psuid,bs="re",by=dummy), 
...
fit_mfi <- bind_rows(fit_msp1ci,fit_nieci,fit_t24ci)
...
# Pf Malaria MSP-1
fitp_msp1 <- mgcv::gam(seropos~s(age,bs="cr",k=11) + s(psuid,bs="re",by=dummy), family="binomial",
                      data=filter(dl,antigen=="pfmsp19"))
newd <- dl %>% filter(antigen=="pfmsp19") %>% mutate(dummy=0)
...
fit_seroprev <- bind_rows(fitp_msp1ci,fitp_nieci,fitp_t24ci)
...
dcorr <- dlc %>%
  ungroup() %>%
  group_by(antigen) %>%
  mutate(corxy=cor(meanmfi,prevroc,method="pearson",use="pairwise.complete.obs") ) %>%
  slice(1)
...
pmfivprev <- ggplot(data=dlc,aes(x=meanmfi,color=antigen)) +
  ...
  geom_text(data=dcorr,
            aes(x=3.5,y=0.1,label=paste("r ==",sprintf("%1.2f",corxy)) ),
  ...
  labs(x="cluster mean log10 MFI",y="cluster seroprevalence (%)") +
  theme_minimal() +
  theme(legend.position="none")
pmfivprev
