...
pmfivkk <- ggplot(data=dlckk,aes(x=prevkk,color=antigenf)) +
  facet_grid(~antigenf) +
  geom_point(aes(y=meanmfi),alpha=0.7) +
  geom_smooth(aes(y=meanmfi),method="loess",color="gray40",lwd=0.5,se=FALSE) +
  geom_text(data=dcorrkk,
            aes(x=0,y=4,label=paste("rho ==",sprintf("%1.2f",cormukk)) ),
            parse=TRUE,col="grey30",hjust=0) +
  scale_x_continuous(breaks=seq(0,0.6,by=0.2),labels=seq(0,60,by=20))+
  scale_color_manual(values=pcols)+
  coord_cartesian(xlim=c(0,0.6),ylim=c(1,4.5)) +
  labs(y="community mean log10 MFI",x="community prevalence by Kato-Katz (%)") +
  theme_minimal() +
  theme(legend.position="none")
pmfivkk
...
# KK mean versus MFI
pmfivkkmean <- ggplot(data=dlckk,aes(x=meankk,color=antigenf)) +
  facet_grid(~antigenf) +
  geom_point(aes(y=meanmfi),alpha=0.7) +
  geom_smooth(aes(y=meanmfi),method="loess",color="gray40",lwd=0.5,se=FALSE) +
  geom_text(data=dcorrkk,
            aes(x=0,y=4,label=paste("rho ==",sprintf("%1.2f",cormukkmu)) ),
            parse=TRUE,col="grey30",hjust=0) +
  scale_x_continuous(breaks=seq(0,1,by=0.2))+
  scale_color_manual(values=pcols)+
  coord_cartesian(xlim=c(0,1.1),ylim=c(1,4.5)) +
  labs(y="community mean log10 MFI",x="community mean log10 eggs per gram (Kato-Katz)") +
  theme_minimal() +
  theme(legend.position="none")
pmfivkkmean
...
pspvkk <- ggplot(data=dlckk,aes(x=prevkk,color=antigenf)) +
  facet_grid(~antigenf) +
  geom_point(aes(y=prevroc),alpha=0.7) +
  geom_smooth(aes(y=prevroc),method="loess",color="gray40",lwd=0.5,se=FALSE) +
  ...
  labs(y="community seroprevalence (%)",x="community prevalence by Kato-Katz (%)") +
  theme_minimal() +
  theme(legend.position="none")
pspvkk
