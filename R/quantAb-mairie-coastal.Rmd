...
dl <- dl %>%
  mutate(antigenf=factor(antigen,levels=c("ashb","nie","sea"),
                         labels=c("Ascaris spp. AsHb","S. stercoralis NIE","S. mansoni SEA")))
...
fit_mfi <- bind_rows(fit_ashbci,fit_nieci,fit_seaci)
...
fit_seroprev <- bind_rows(fitp_ashbci,fitp_nieci,fitp_seaci)
...
foi_ests <- foreach(ai=c("ashb","nie","sea"),.combine=rbind) %:%
  foreach(comi=levels(dl$community),.combine=rbind) %do% {
  
  di <- dl %>% filter(antigen==ai & community==comi)
  ...
  res <- data.frame(community=comi,antigen=ai,lambda,lambda_lb,lambda_ub)
  return(res)
}
...
d_foi <- dlc %>%
  left_join(foi_ests,by=c("community","antigen")) %>%
  mutate(antigen=factor(antigen,levels=c("ashb","nie","sea")))
...
Figure of results
```{r foi vs mfi figure}
pfoivmfi <- ggplot(data=d_foi,aes(x=meanmfi,y=lambda,color=antigen)) +
  facet_grid(~antigen)+
...
pfoivprev <- ggplot(data=d_foi,aes(x=prevroc,y=lambda,color=antigen)) +
  facet_grid(~antigen)+
  geom_point(alpha=0.7) +
  geom_smooth(method="glm",color="gray40",lwd=0.5,se=FALSE) +
  geom_text(data=dfoicorr,
