...
pdistashb <- ggplot(data=filter(dl,antigen=="ashb"),aes(x=logmfi,fill=antigen)) +
  ...
  labs(x="luminex response (MFI-bg)", title="Ascaris spp. AsHb") +
  theme_minimal() + 
...
pmfivprev2 <- ggplot(data=dlc,aes(x=meanmfi,color=antigen)) +
  facet_grid(~antigen) +
  geom_point(aes(y=prevroc),alpha=0.7) +
  geom_smooth(aes(y=prevroc),method="glm",color="gray40",lwd=0.5,se=FALSE) +
  geom_text(data=dcorr,
